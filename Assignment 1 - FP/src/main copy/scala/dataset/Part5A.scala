...
//    println("Q26:\n" + highestCheckouts(patrons))
  ...
  def highestCheckouts(xs: List[Array[String]]): Tuple3[Int, String, String] =
    xs.sortBy(x => -x(2).toInt)
      .map((x: Array[String]) => (x(0).toInt, x(4), x(8)))
      .head

  /** Q27 (3p)
   * Output the patron type and number of patrons for each type, as a map
   * Make sure the patrons satisfy the following condition:
  ...
  def missing_email(xs: List[Array[String]]): Map[String, Int] =
    xs.filter(p => p(8) == "email" && p(9) == "false")
      .groupBy(_(1))
      .mapValues(_.size)
