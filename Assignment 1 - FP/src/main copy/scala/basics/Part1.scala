...
object Part1 {
   ...
   * reassigned. If you use `var` you can reassign its value though.
   ...
   * immutability.
   *
   * Within this lab assignment you are *NOT* allowed to use
   ...
   * assigned to a variable, i.e. the anonymous function (or lambda
   * expression as it is also called) can be written as follows:
   *
   * (i: Int) => i * 5 + 2
   *
   * This can be assigned to variable called increaseGrade by:
   *
   ...
   * functions that take functions as input parameters, replace the ??? of the
   * anonymous functions with methods bodies and provide the correct
   * function signature for myFirstHof and mySecondHof in the method body of
  ...
  def question2(): List[List[Int]] = {
      ...
      // - Uncomment the entire body of this function (select all
      //   between the curly braces, then press Ctrl + /)
      // - Remove the placeholder List(List(1), List(2), List(3))
  		// - Fill in the parameter type of f in both HOFs below,
  		//   keeping in mind the expected input and output types

  		def myFirstHOF(xs: List[Int], f: Int => Boolean) : List[Int] = xs.filter(f)
  		def mySecondHOF(xs: List[Int], f: Int => Int ) : List[Int] = xs.map(f)
