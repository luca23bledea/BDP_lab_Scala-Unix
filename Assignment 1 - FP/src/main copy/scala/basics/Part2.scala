...
object Part2 {
   ...
   * definition. Case classes allow for pattern matching. Instead of an if
  ...
  def drunkWords(xs: List[String]): List[String] = {
    @annotation.tailrec
    def helper(list: List[String], acc: List[String]): List[String] = {
      list match {
        case Nil => acc
        case h :: t => helper(t, h.reverse :: acc)
      }
    }
   ...
   * check if some condition holds for ALL elements of the list. An empty
   ...
   * @return None if the list is empty or Some( _ : A), the last element of
   *         the list
   *
   *         Examples:
   *         lastElem(List()) // None
   ...
   * ... it has a twist: It should also throw out each odd-indexed list
   * element which satisfy 'f' (i.e. it should keep the even-indexed list
   * elements which satisfy 'f').
   *
  ...
  def myFilter[A](xs: List[A], f: A => Boolean): List[A] = {
    def helper(list: List[A], i: Int, acc: List[A]): List[A] = {
      list match {
