 ...
 * What the foldLeft does in this example is: it starts with a fixed value
 * called an accumulator (in this case, 0), then it takes from left to right,
 * one element from the list and feeds it as a parameter into the function,
 * essentially piling each element onto the accumulator, updating it every step
 * of the way. The function specifies the operation for combining the
 * accumulator with each element.
 * In this case `_ + _` is the shorthand for ( (x: Int, y:Int) => x + y) 
 * Some people call this reduce, but that is not correct. While similar
 ...
 *
 * As you can see there are some minor, but tricky differences. Fold takes
 * an initial argument as the accumulator, while reduce uses the first/last
 * element as the accumulator and continues from there.
...
object Part4 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println("Q17:\n" + secondLargest(List(1,2,3,4,5,6)) + "\n" + secondLargest(List(1,1,1,1)))
    println("Q18:\n" + countNumbers(List(1,2,1,2,1,2,3,4,4,4,4,4,4,4,4)))
    println("Q19:\n" + distinctAPI(List(1,2,1,2,1,2,3,4,4,4,4,4,4,4,4)))
    println("Q20:\n" + specialPartition(List(List(-4,1,2), List(-4,-4,-6,1,2,4,4), List(-4,-4, -6, 1,2,8))))
    println("Q21:\n" + determineGoodPlayers(tudScrabblePlayers))
    println("Q22:\n" + avgOver7(tudScrabblePlayers))
    ...
    println("Q23:\n" + goodSportStudents(asr, ss))
    println("Q24:\n" + avgGradeGoodSportStudents(asr, ss))
  }
   ...
   */
  def reverseUsingFold[A](nrs: List[A]): List[A] = nrs.foldLeft(List.empty[A])((acc, a) => a :: acc)
  ...
  def secondLargest(xs: List[Int]): Option[Int] = {
    @annotation.tailrec
    def helper(xs: List[Int], max1: Option[Int], max2: Option[Int]): Option[Int] = {
      xs match {
        case Nil => max2
        case h :: t =>
          max1 match {
            case None => helper(t, Some(h), None)
            case Some(num1) if(h > num1) => helper(t, Some(h), Some(num1))
            case Some(num1) if(h < num1) =>
              max2 match {
                case None => helper(t, max1, Some(h))
                case Some(num2) if(num2 < h) => helper(t, max1, Some(h))
                case _ => helper(t, max1, max2)
              }
            case _ => helper(t, max1, max2)
          }
      }
   ...
   */
  def countNumbers(xs: List[Int]): Map[Int, Int] = xs.groupBy(identity).mapValues(_.size)
  ...
  /** Q20 (3p)
   * Special partition takes all numbers across all lists, adds one to the negative numbers
   * and checks if all those summed up have the same absolute value as all the positive numbers
   *
   * Example:
   * specialPartition(List(List(-4,1,2), List(-4,-4,-6,1,2,4,4), List(-4,-4, -6, 1,2,8))) // true
   */
  def specialPartition(xs: List[List[Int]]): Boolean = {
    val (sumNegative, sumPositive) = xs.flatten.foldLeft(0,0) {
      case((neg, pos), x) =>
        if(x < 0)
          (neg + x + 1, pos)
        else
    ...
    math.abs(sumNegative) == sumPositive
  }
  ...
  def determineGoodPlayers(scrabblePlayers: List[(String, List[(String, Int)])]): List[String] =
    scrabblePlayers
      .filter(list => !list._2.isEmpty)
      .filter(list => list._2.forall(word => word._1.length > 3))
      .map(name => name._1)

  /** Q22 (3p)
   * Determine which of the GOOD Scrabble players have an average value per word of over 7
  ...
  def avgOver7(scrabblePlayers: List[(String, List[(String, Int)])]): List[(String, Double)] = {
    ...
    scrabblePlayers
      .filter { case(name, _) => goodPlayers.contains(name)}
      .map {
        case(name, words) =>
        val avg = words.map(_._2).sum / words.length.toDouble
          (name,avg)
      }
      .filter { case(_, avg) => avg > 7.0}
  }

  /** Q23 (5p)
   * Given an anonymised list of students who do sport and another list of
  ...
  def goodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[String] = {
    studentResults
      .filter { case(name, _, _) => sportStudents.contains(name)}
      .groupBy(students => students._1)
      .collect {
        case(name, grade)
          if grade.flatMap(_._3).size == grade.size &&
          grade.flatMap(_._3).forall(_ >= 6) => name
      }
      .toList
  }
  ...
  def avgGradeGoodSportStudents(studentResults: List[(String, String, Option[Int])], sportStudents: List[String]): List[(String, Double)] = {
    ...
    studentResults
      .filter{ case(name, _, _) => goodStudents.contains(name) }
      .groupBy(_._1)
      .map {
        case(name, grades) =>
          val avg = grades.flatMap(_._3).sum / grades.size.toDouble
          (name, avg)
      }
      .toList
  }
