 ...
 *
 * Well, that's easy, though it takes a bit of practice. You'll see that this
 * technique of chaining operations is the heart of big data processing, as it
 * will come back in the form of Unix pipelines.
 *
 * If you use IntelliJ you'll have the advantage that types will be placed in 
 * comment popups on the right side of your code, but if that's not working for
 ...
 *   .range(0, 11)             		// range(0,11) returns List(0,1,2,3,4,...,10) List.range 2nd arg is exclusive
 ...
 * each transformation. This will be beneficial when you reach the Spark/Flink
 * assignments.
 *
 ...
 * Note: those fancy operators such as ::, ::: or :+ also count as API
 * methods; they are just syntactic sugar for regular methods
 * 
...
object Part3 {
  /** Q10 (2p)
   */
  def twiceAPI[A](xs: List[A]): List[A] = xs.flatMap(x => List(x, x))


  /** Q11 (2p)
   */
  def drunkWordsAPI(xs: List[String]) = xs.map(x => x.reverse).reverse

  /** Q12 (2p)
   */
  def myForAllAPI[A](xs: List[A], f: A => Boolean): Boolean = xs.forall(f)

  /** Q13 (2p)
   * implement `lastElem` only using API calls
   */
  def lastElemAPI[A](xs: List[A]): Option[A] = xs.lastOption

  /** Q14 (2p)
   * implement `append` only using API calls
   */
  def appendAPI[A](xs: List[A], ys: List[A]): List[A] = xs ::: ys

  /** Q15 (2p)
   * implement `intList` (from part 1) only using API calls
   */
  def intListAPI(a: Int, b: Int) : List[Int] = List.range(a, b + 1)
  // END OF PART 3

  // Main for debug purposes
  def main(args: Array[String]): Unit = {
    println("Q10:\n", twiceAPI(List(0, 1, 2, 3)))
    println("Q11:\n" + drunkWordsAPI(List("Hey","you,","how","are","you","doing?")))

    val startsWithS = (s: String) => s.startsWith("s")
    println("Q12:")
    println(myForAllAPI(List("abc", "def"), startsWithS))
    println(myForAllAPI(List("start", "strong", "system"), startsWithS))

    println("Q13:")
    println(lastElemAPI(List())) // None
    println(lastElemAPI(List.range(0,3))) // Some(2) (range has exclusive ceiling)

    println("Q14:")
    println(appendAPI(List(), List()))	// List()
    println(appendAPI(List(1,3,5), List(2,4))) 	// List(1,3,5,2,4)
